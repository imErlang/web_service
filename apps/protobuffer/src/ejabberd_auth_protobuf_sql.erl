%%%----------------------------------------------------------------------
%%% File    : ejabberd_auth_protobuf_sql.erl
...
-module(ejabberd_auth_protobuf_sql).
...
-include_lib("ejabberd/include/ejabberd_sql_pt.hrl").
-include_lib("ejabberd/include/ejabberd_auth.hrl").
...
get_password(User, Server) ->
    UserInfo =  'Elixir.Ejabberd.HostUsers':find_user(User, Server),
    case UserInfo =/= nil of
        true ->
            {cache, {ok, maps:get(password, UserInfo)}};
        false ->
            {nocache, error}
    end.
